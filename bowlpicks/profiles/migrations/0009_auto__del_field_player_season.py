...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'Player.season'
        db.delete_column('profiles_player', 'season_id')

        # Adding M2M table for field season on 'Player'
        db.create_table('profiles_player_season', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('player', models.ForeignKey(orm['profiles.player'], null=False)),
            ('season', models.ForeignKey(orm['core.season'], null=False))
        ))
        db.create_unique('profiles_player_season', ['player_id', 'season_id'])


    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'Player.season'
        raise RuntimeError("Cannot reverse this migration. 'Player.season' and its values cannot be restored.")
        # Removing M2M table for field season on 'Player'
        db.delete_table('profiles_player_season')
